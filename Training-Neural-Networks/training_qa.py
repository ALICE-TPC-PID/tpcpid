...
print(f"DEBUG: Using Hadronic Rate option {HadronicRateString} in training_qa_251017")
HadronicRateBool = HadronicRateString.strip().lower() == "true"
print(f"Hadronic Rate value {HadronicRateString} was casted to bool {HadronicRateBool}")
...
def QA2D_NSigma_vs_Var(i, mass, plot_against = 'fTPCInnerParam', log_x = True, range_hists = [[-1.,1.]]*6, fitted_particles=['Electrons', 'Pions', 'Kaons', 'Protons', 'Deuterons', 'Tritons'], bins_sig_mean=100, sigma_range = [-3.,3.], useNN = True, xlabel = r'p [GeV/c]', transform_x = lambda x: x, plot_mode="nsigma"):
    ...
    print("Creating histograms for", fitted_particles[i], "against", plot_against, "for mode:", plot_mode)
    ...
    if(np.sum(mask_pred)!=0):
        ...
        if plot_mode == "nsigma":
            ...
            if useNN:
                ...
                print("Relative sigma plot only available for NN mode")
                return
            ...
            print("Unknown plot output:", plot_mode)
            return
        ...
        for j in range(bins_sig_mean):
            try:
                ...
                if log_x:
                    print("Exception at: ", particles[pid_idx], "/", plot_against,
                    "for bins:", 10**bin_edges[j], 10**bin_edges[j+1])
                else:
                    print("Exception at: ", particles[pid_idx], "/", plot_against,
                    "for bins:", bin_edges[j], bin_edges[j+1])
                print(e)
                binned_mean[0][j], binned_sigma[0][j] = np.nan, np.nan
                continue
        ...
        if "raw_relsigma" not in plot_mode:
            ...
            if positive_counts.size:
                ...
                print(f"WARNING: Skipping LogNorm colorbar for {particles[pid_idx]} vs {plot_against} (no histogram entries)")
            if log_x:
        ...
        plt.savefig(output_dir + "/" + plot_against + "/" + output_name + particles[pid_idx] + "_" + plot_against + '.pdf', bbox_inches='tight')
        plt.close()
...
if HadronicRateBool:
    os.system("rm -rf " + output_dir + "/fHadronicRate")
    os.makedirs(output_dir + "/fHadronicRate")
    print("DEBUG: Training QA 251017: Created folder for QA plots for HadronicRate")
else:
    print("DEBUG: Training QA 251017: No folder for QA plots for HadronicRate (disabled)")
...
for i, mass in enumerate(np.sort(np.unique(fit_data[:,labels=='fMass'].flatten()))):
    ...
    val = CONFIG['trainNeuralNetOptions']['isSmallSystem']
